...
export default function DriversPage() {
    ...
    return (
        <main className="min-h-screen bg-[#121212] text-white">
            <Navbar />            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <h1 className="text-3xl md:text-5xl font-bold mb-4 md:mb-0" style={textShadowStyle}>F1 Drivers</h1>
                ...
                <div className="bg-yellow-900/40 backdrop-blur-sm border border-yellow-700/50 text-yellow-200 px-5 py-4 rounded-lg mb-6 flex items-center shadow-lg">
                    <svg className="h-6 w-6 mr-3 text-yellow-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex flex-col justify-center items-center h-64">
                    <div className="relative">
                        ...
                        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-r-2 border-purple-400 absolute top-2 left-2"></div>
                        <div className="animate-pulse absolute top-5 left-5 h-6 w-6 bg-purple-600 rounded-full"></div>
                    </div>
                    <p className="mt-4 text-gray-400">Loading drivers...</p>
                </div>
            ) : selectedDriver ? (<div className="mb-6">
                {/* Driver Detail View */}
                <button
                    onClick={() => setSelectedDriverId(null)}
                    className="mb-6 bg-gray-800 hover:bg-gray-700 text-white py-2 px-4 rounded-md transition-colors duration-200 flex items-center"
                >
                    ...
                    <div className="relative h-64 md:h-96 bg-gradient-to-r from-black via-black/70 to-transparent">                        {selectedDriver.imageUrl && !failedImageUrls.has(selectedDriver.imageUrl) ? (
                        ...
                        <div className="relative z-10 h-full flex flex-col justify-end p-6">
                            <div className="flex flex-wrap items-center gap-2 mb-3">
                                <span className="bg-white/10 backdrop-blur-sm rounded-md px-3 py-1 text-sm font-medium">
                                    #{selectedDriver.number || "N/A"}
                                </span>
                                <span className="bg-white/10 backdrop-blur-sm rounded-md px-3 py-1 text-sm font-medium">
                                    <span className="mr-1 text-base">{getCountryFlag(selectedDriver.nationality)}</span> {selectedDriver.nationality}
                                </span>
                                {selectedDriver.position && (
                                    <span className="bg-purple-900/30 backdrop-blur-sm rounded-md px-3 py-1 text-sm font-medium flex items-center">
                                        <span className="mr-1">P{selectedDriver.position}</span> in standings
                                    </span>
                                )}
                            </div>                                    <h2 className="text-4xl md:text-6xl font-bold" style={textShadowStyle}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="bg-[#1A1A1A]/50 rounded-xl p-5 border border-gray-800/20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {currentDrivers.map((driver) => (
                        ...
                        >                            <div className="h-52 relative overflow-hidden">
                            ...
                            <div className="p-4">
                                <p className="text-gray-400 text-sm mb-3">{driver.name}</p>

                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-400 flex items-center">
                                        <span className="mr-1 text-base">{getCountryFlag(driver.nationality)}</span>
                                        {driver.nationality}
                                    </span>
                                ...
                                <div className="mt-3 pt-3 border-t border-gray-800/50">
                                    <div className="flex justify-between items-center">
                                        <div>
                                            ...
                                            <p className="font-medium truncate text-sm">{driver.team || "Unknown"}</p>
                                        </div>                                            {driver.wins !== undefined && driver.wins > 0 && (
                                            <div className="flex items-center bg-yellow-900/20 px-2 py-1 rounded-md">
                                                <Trophy className="h-3 w-3 mr-1 text-yellow-500" />
                                                <span className="text-xs text-yellow-200">{driver.wins}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
